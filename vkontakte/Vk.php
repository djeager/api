...
require_once $_SERVER['DOCUMENT_ROOT'] . "/vendor/djeager/parser/extensions/api/vk/vkPhpSdk" . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'VkPhpSdk.php';
require_once $_SERVER['DOCUMENT_ROOT'] . "/vendor/djeager/parser/extensions/api/vk/vkPhpSdk" . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'Oauth2Proxy.php';
...
{  
    ...
    private $_redirectUri =    'http://za-repost.com/admin/parser/vk/newtoken';      // redirect url
...
//			session_start();
	...
	{		
        if( is_array($e) && isset($e['error'])){
                $err=new \backend\modules\post\extensions\api\vk\Error;
    ...
    public function validAttachments($a, $p)
    {
        ...
        {
            if(is_object($v)) {$aa[$k] = $v; continue;}
            $name = ucfirst($v['type']);
            $name = "\\vendor\\djeager\\parser\\extensions\\api\\vk\\" . $name;
            //            $objname =
